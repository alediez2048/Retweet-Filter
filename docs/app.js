/**
 * Standalone Dashboard for Retweet Filter
 * Uses IndexedDB directly instead of Chrome extension APIs
 */

import { db } from './lib/db.js';
import { searchRetweets } from './lib/search.js';
...
async function init() {
  console.log('[Dashboard] Initializing standalone version...');
...
// ==================== DATA LOADING (Direct DB) ====================

async function loadStats() {
  try {
    const stats = await db.getStats();
    if (elements.totalRetweets) elements.totalRetweets.textContent = formatNumber(stats.total);
    if (elements.todayRetweets) elements.todayRetweets.textContent = formatNumber(stats.today);
  } catch (error) {
...
async function loadRetweets() {
  ...
  try {
    const allData = await db.getAllRetweets();

    // Apply platform filter
    let filtered = allData;
    if (currentFilters.platform) {
      filtered = filtered.filter(r => r.platform === currentFilters.platform);
    }

    if (query || Object.keys(currentFilters).length > 0) {
      const results = searchRetweets(filtered, query, currentFilters);
      allRetweets = results.map(r => r.item);
    } else {
      // Sort by captured_at descending
      allRetweets = filtered.sort((a, b) =>
...
async function loadSavedSearches() {
  try {
    ...
    if (elements.savedSearches) {
      elements.savedSearches.innerHTML = searches.map(search => `
...
async function saveCurrentSearch() {
  ...
  await db.saveSearch({ name, query, filters: currentFilters });
  loadSavedSearches();
}
...
async function saveCategory() {
  ...
  try {
    if (editingCategoryName && editingCategoryName !== name) {
      await db.deleteCategory(editingCategoryName);
    }
...
async function saveTags() {
  ...
  try {
    await db.updateTags(editingRetweetId, tags);
    closeTagModal();
    loadRetweets();
  } catch (error) {
...
function openBulkTagModal() {
  ...
  if (saveBtn) {
    saveBtn.onclick = async () => {
      ...
      await db.bulkUpdateTags(Array.from(selectedIds), tags, []);
...
async function bulkDelete() {
  ...
  try {
    await db.deleteRetweets(Array.from(selectedIds));
    clearSelection();
    loadRetweets();
    loadStats();
  } catch (error) {
...
function openDetailModal(id) {
  ...
  body.innerHTML = `
    ...
    <div style="display:flex;gap:12px">
      <a href="${escapeHtml(retweet.source_url || '#')}" target="_blank" class="primary-btn">Open Original</a>
...
async function handleArchiveImport(e) {
  ...
  try {
    ...
    // Parse X archive format (tweets.js)
    let tweets = [];
    if (text.startsWith('window.YTD.tweets.part0')) {
    ...
    showImportResults({ success: true, data: result });
  } catch (error) {
...
async function handleCsvImport(e) {
  ...
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      retweets.push({
        tweet_id: row.id || row.tweet_id || `csv_${i}`,
        text: row.text || row.content || '',
        user_handle: row.handle || row.username || row.user_handle || '',
...
async function handleNitterImport() {
  ...
  alert('Nitter import requires server-side processing. Please use the Chrome extension for this feature.');
}
